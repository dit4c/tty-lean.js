...
function Server(conf) {
  ...
  this.on('listening', function() {
    ...
    if (typeof(address) == 'object') {
      self.log('Listening on port \x1b[1m%s\x1b[m.', address.port);
    } else {
      self.log('Listening on socket \x1b[1m%s\x1b[m.', address);
    }
  });
...
Server.prototype.initIO = function() {
  ...
  io.sockets.on('connection', function(socket) {
    self.log('Incoming connection from socket %s', socket.id);
...
Server.prototype.handleConnection = function(socket) {
  ...
  socket.on('start', function(startId, cb) {
    var session = new Session(self, socket, startId, cb || function() {});
...
Server.prototype.listen = function() {
  // If no arguments given, use config
  if (arguments.length === 0) {
    if (this.conf.socket) {
      // Listen on Unix socket
      if (fs.existsSync(this.conf.socket)) {
        fs.unlinkSync(this.conf.socket);
      }
      return this.server.listen(this.conf.socket);
    } else {
      // Listen on TCP port
      return this.server.listen(this.conf.port, this.conf.hostname);
    }
  }
  // Otherwise, delegate the problem
  return this.server.listen.call(this.server, arguments);
};
...
function Session(server, socket, startId, callback) {
  this.server = server;
  this.socket = socket;
  this.terms = {};
  ...
  this.id = this.uid(startId);

  // TODO: Implement reconnection rather than disconnect
  ...
  if (conf.syncSession) {
    ...
    if (stale) {
      if (stale.socket.disconnected) {
        ...
        stale.sync();
        stale.log(
          'Session \x1b[1m%s\x1b[m resumed on socket %s.', stale.id, socket.id);
        callback('resumed', this.id);
        return stale;
      } else {
        this.id += '~';
        stale.log(
          'Session \x1b[1m%s\x1b[m already connected to socket %s. ' +
          'Rejecting socket %s.', stale.id, stale.socket.id, socket.id);
        callback('conflict', stale.id);
        socket.disconnect();
        return this;
      }
    }
  ...
  this.log(
    'Session \x1b[1m%s\x1b[m created for socket %s.', this.id, socket.id);
  callback('started', this.id);
}

// TODO: establish stable identifiers, as port isn't stable behind reverse proxies.
Session.uid = 0;
Session.prototype.uid = function(startId) {
